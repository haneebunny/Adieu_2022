...
export default function Tag(props: any) {
  const onKeyPress = (e: any) => {
    const hashtag = e.target.value.replace(
      e.target.value,
      `#${e.target.value}`
    );
    if (e.key === "Enter") {
      ...
      if (props.tagList === undefined) {
        props.setTagList([hashtag]);
        props.setTagItem("");
      } else {
        //추가로 입력한 값
        props.setTagItem("");
        const duplicateTagList = props.tagList.filter(
          (tagItem: string) => tagItem === hashtag
        );
        if (duplicateTagList.length > 0) return;
        props.setTagList((prev: any) => [...prev, hashtag]);
      }
    }
  };
